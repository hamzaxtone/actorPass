   ...
   $ext = strtolower(pathinfo($file_path, PATHINFO_EXTENSION));
    ...
    <div class="path">
        ...
        <p>
			...
			<b><a href="#edit" class="edit_btn"><i class="icon-edit"></i> Edit</a></b>&nbsp;
			<b><a href="#save" data-url="<?php echo $file_path ?>" class="save_btn disabled"><i class="icon-edit"></i> Save</a></b>&nbsp;
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
	...
	textarea[name=content_file]{
		width:100%;
		height:50px;
	}
	#myeditor textarea[name=content_file]{
		position:absolute;
		top:0;
		opacity:0;
	}
...
<?php
if( isset($_POST["content_file"]) && isset($_POST["content_file_url"]) ){
	echo $content_file = $_POST['content_file'];
	echo $content_file_url = $_POST['content_file_url'];
	/* $dt = new DateTime($s);
	$last_saved_files = array
	(
	array($dt,$content_file,$content_file_url)
	);
	$_SESSION['last_saved_files'] = $last_saved_files; 
	print_r($_SESSION['last_saved_files']); */
 
	
$fw = fopen($content_file_url, 'w') or die('Could not open file!');
$fb = fwrite($fw,stripslashes($content_file)) or die('Could not write to file');
if( $fb ){
/* header('Content-Type: application/json');
echo json_encode(array('status' => 'File Successfully Saved'));	 */
}
fclose($fw);
 }
 
                $content = '<pre class="with-hljs asd"><code class="' . $hljs_class . '">' . fm_enc($content) . '</code></pre>
			<span class="file_save_status">Successfully File Saved</span>	
			<section id="editorsection">
				<div id="editor">
				' . fm_enc($content) . '
				</div>
				<textarea name="content_file" row="5" cols="5">' . fm_enc($content) . '</textarea>
				</section>
...
$(".edit_btn").click(function(){
 ...
 $(".save_btn").click(function(){
	var content_file = $("textarea[name=content_file]").val();		
	var content_file_url = $(this).data("url");		
			
$.ajax({
    type: "POST",
    url: window.location.href,
    data: {content_file: content_file, content_file_url: content_file_url },
	action: "update_file",
    success: function(data){
        $(".file_save_status").fadeIn();
		//alert("Successfully File Saved");
		$(".file_save_status").fadeOut("999999999");
    },
	error: function(data){
        alert(data);
    }
   ...
   var textarea = $("textarea[name=content_file]");
    ...
    $('.search_bar')
      .change( function () {
        ...
        if(filter) {
          ...
          $(list).find("tr td:not(:Contains(" + filter + "))").parent().slideUp();
          $(list).find("tr td:Contains(" + filter + ")").parent().slideDown();
        } else {
...
<?php if (FM_USE_AUTH): ?><a title="Logout" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
		...
		$search_bar = "<input type='search' class='search_bar' style='margin-left:10px;width:50%;'/>";
        ...
        echo '<div class="break-word">' . $root_url .$search_bar. '</div>';
		//echo '<div class="break-word">' . $search_bar . '</div>';
        ?>
</div>
<?php
}
